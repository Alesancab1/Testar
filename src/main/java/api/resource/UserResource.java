...
import aiss.model.repository.UserRepositoryImpl;
...
public class UserResource {
	...
	public Response addUser(@Context UriInfo uriInfo, User u) {
		...
		if (u.getEdad() == null || u.getEdad() < 0.0)
			throw new BadRequestException("Invalid Edad");
	...
	public Response update(@PathParam("userID") String id, User u) {
		...
		if (!old.getId().equals(u.getId()) || 
			!old.getNickname().equals(u.getNickname()))
			return Response.status(Status.CONFLICT).build();
		
		if (u.getEdad() < 0.0)
			throw new BadRequestException("The new Edad must be " + 
										   "a positive value");
